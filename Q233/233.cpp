...
public:
    int countDigitOne(int n) {
        int ans = 0;
        for(long long m = 1;m <= n;m *= 10){// beware of overflow
            int a = n/m, b = n%m;
...
For each position, split the decimal representation into two parts, for example split n=3141592 into a=31415 and b=92 when we're at m=100 for analyzing the hundreds-digit. 
